...
if 'YouTubeLiveSnapshot.download(' not in src:
    ...
    replacement = '''        val wantedHeight = quality.maxHeight ?: Int.MAX_VALUE\n        val snapshot = YouTubeLiveSnapshot.download(\n            YouTubeLiveSnapshot.Request(\n                url = url, wantedHeight = wantedHeight, proxy = proxy, knownStartTimestamp = liveStartTimestamp,\n                ytDlp = ytDlp, ffmpeg = ffmpeg, binDir = binDir, deno = deno, appDir = appDir.toFile(), videosDir = videosDir\n            ),\n            onProcess = { activeProcess.set(it) },\n            update = update\n        )\n        if (snapshot.isSuccess) {\n            update(100, "Live salva do início até o ponto em que o download foi iniciado.")\n            return snapshot.getOrThrow()\n        }\n        update(0, "O snapshot HLS não ficou disponível. Tentando modo compatível do yt-dlp…")\n        val selector = quality.maxHeight?.let { "b[height<=$it]/b" } ?: "b"\n        val nowSeconds = System.currentTimeMillis() / 1000L\n'''
    if marker in src:
        src = src.replace(marker, replacement, 1)

# Atualiza árvores já materializadas por execuções anteriores do patch.
src = src.replace('val wantedHeight = quality.maxHeight ?: 2160', 'val wantedHeight = quality.maxHeight ?: Int.MAX_VALUE')
old_unbounded = '''        val extra = mutableListOf("--live-from-start", "--hls-use-mpegts")\n        if (targetSeconds != null && targetSeconds > 0L) {\n            val end = formatLiveTime(targetSeconds)\n            update(0, "🔴 Live detectada. Baixando do início até o ponto atual ($end)...")\n            extra += listOf(\n                "--download-sections", "*00:00:00-$end",\n                "--force-keyframes-at-cuts",\n                "--concurrent-fragments", "4"\n            )\n        } else {\n            update(0, "🔴 Live detectada. Baixando do início até o ponto atual...")\n        }\n        return runYtDlp(url, selector, proxy, extra, update)'''
new_bounded = '''        if (targetSeconds == null || targetSeconds <= 0L) {\n            error("Não foi possível determinar o ponto atual da live com segurança. O download foi interrompido para não acompanhar a transmissão indefinidamente.")\n        }\n        val end = formatLiveTime(targetSeconds)\n        update(0, "🔴 Live detectada. Baixando do início até o ponto atual ($end)...")\n        val extra = mutableListOf(\n            "--live-from-start", "--hls-use-mpegts",\n            "--download-sections", "*00:00:00-$end",\n            "--force-keyframes-at-cuts",\n            "--concurrent-fragments", "4"\n        )\n        return runYtDlp(url, selector, proxy, extra, update)'''
if old_unbounded in src:
    src = src.replace(old_unbounded, new_bounded, 1)
...
assert 'quality.maxHeight ?: Int.MAX_VALUE' in updated
assert 'para não acompanhar a transmissão indefinidamente' in updated
assert 'quality.maxHeight ?: 2160' not in updated
print('YouTube Live: snapshot HLS congelado + fallback temporal estritamente limitado ao ponto atual.')
