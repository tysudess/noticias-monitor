...
internal class GloboplayLoginWindow(
    ...
    private fun buildAndShow() {
        ...
        saveButton.addActionListener {
            val netscape = buildNetscapeCookieJar(cookieManager.cookieStore.cookies)
    ...
    private fun buildNetscapeCookieJar(cookies: List<HttpCookie>): String {
        val now = System.currentTimeMillis() / 1000L
        val body = cookies.asSequence()
            .filter { cookie ->
                val domain = cookie.domain.orEmpty().lowercase()
                domain.contains("globo.com") || domain.contains("globoplay")
            }
            .filter { it.name.isNotBlank() }
            .joinToString("\n") { c ->
                val domain = c.domain?.takeIf { it.isNotBlank() } ?: ".globoplay.globo.com"
