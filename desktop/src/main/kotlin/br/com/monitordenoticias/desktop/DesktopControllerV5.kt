...
class DesktopControllerV5(
    ...
    private fun migrateProxyHost() {
        val savedHost = prefs.getString("desktop_proxy_host", "").orEmpty().trim()
        if (savedHost.equals("proxy-7db.mb", ignoreCase = true) || savedHost.isBlank()) {
            prefs.edit().putString("desktop_proxy_host", "proxy-7dn.mb").apply()
        }
    }
