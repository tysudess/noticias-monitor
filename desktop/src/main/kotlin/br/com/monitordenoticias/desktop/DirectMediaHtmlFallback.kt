...
internal object DirectMediaHtmlFallback {
    fun extractCandidates(pageUrl: String, proxyUrl: String): List<String> = runCatching {
        ...
        val absolute = Regex(
            "https?://[^\\s\\\"'<>]+?(?:\\.m3u8|\\.mp4|\\.m4v|\\.webm|\\.mov)(?:\\?[^\\s\\\"'<>]*)?",
            RegexOption.IGNORE_CASE
        ).findAll(html).map { it.value }

        val attributed = Regex(
            "(?:src|href|url|file|contentUrl)\\s*[:=]\\s*[\\\"']([^\\\"']+?(?:\\.m3u8|\\.mp4|\\.m4v|\\.webm|\\.mov)[^\\\"']*)",
            RegexOption.IGNORE_CASE
        ).findAll(html)
        ...
        (absolute + attributed)
            .map { it.trim().trim('"', '\'') }
            .filter { isValidMediaUrl(it) }
            .distinct()
            .take(15)
            .toList()
    }.getOrDefault(emptyList())

    fun isDirectMediaUrl(url: String): Boolean = runCatching {
        val path = URI(url).path.lowercase()
        path.endsWith(".m3u8") || path.endsWith(".mp4") || path.endsWith(".m4v") || path.endsWith(".webm") || path.endsWith(".mov")
    }.getOrDefault(false)
    ...
    private fun isValidMediaUrl(url: String): Boolean {
        ...
        if (listOf("doubleclick", "analytics", "pixel", "tracking", "favicon", "logo.", "sprite").any { lower.contains(it) }) return false
